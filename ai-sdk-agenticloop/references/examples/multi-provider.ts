...
class MultiProviderPool {
  ...
  async executeWithFallback(
    ...
    for (const config of sorted) {
      try {
        ...
        console.error(`Failed: ${config.provider}/${config.model}`, error.message)
        continue
      }
    }
  ...
  private async stream(model: any, prompt: string, maxSteps = 10) {
  ...
  private async generate(model: any, prompt: string, maxSteps = 10) {
